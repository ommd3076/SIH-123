...
if ($Distributed) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m robotics_ws.supervisor --seed 7"
    Start-Sleep -Seconds 2
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m robotics_ws.telemetry_bridge"
} else {
    Write-Host "[1/2] Starting lightweight single-process fleet engine (super fast & light)..." -ForegroundColor Green
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "python -m robotics_ws.telemetry_bridge --lightweight"
}
